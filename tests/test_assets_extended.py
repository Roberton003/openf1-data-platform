...
def testwrite_session_partition(tmp_path):
...
def testappend_execution_record(tmp_path):
...
def testappend_execution_record_merges(tmp_path):
    append_execution_record(str(tmp_path), {"run_id": 1})
    append_execution_record(str(tmp_path), {"run_id": 2})
    result = pd.read_parquet(tmp_path / "data.parquet")
    assert len(result) == 2
...
def testcalc_freshness_minutes_with_data(tmp_path):
...
def testcalc_freshness_minutes_nonexistent():
    assert calc_freshness_minutes("/nonexistent/path") is None


def testcalc_freshness_minutes_empty(tmp_path):
    empty = tmp_path / "empty"
    empty.mkdir()
    assert calc_freshness_minutes(str(empty)) is None
