...
def test_silver_metadata_tables_happy_path(tmp_data_dir, mock_fetch_api, mocker):
    ...
    race_control.to_parquet(
        os.path.join(bronze_dir, "race_control.parquet"), index=False
    )
    ...
    session_result.to_parquet(
        os.path.join(bronze_dir, "session_result.parquet"), index=False
    )
...
def test_bronze_sessions_with_data_writes_parquet(tmp_data_dir, mocker):
    ...
    mocker.patch(
        "src.ingestion.assets.fetch_api",
        return_value=[{"session_key": 10014, "year": 2025}],
    )
